...
(function () {
  ...
  function drawRandomCard() {
    ...
    if (mpState.isOnline && mpState.roomRef) {
      if (mpState.isAuctionMode) {
        ...
        mpState.roomRef.update(sanitizeForFirebase(auctionPayload)).catch((err) => {
          console.error('Firebase summon auction error:', err);
          alert('Failed to summon card online: ' + err.message);
        });
      } else {
        ...
        mpState.roomRef.update(sanitizeForFirebase(draftPayload)).catch((err) => {
          console.error('Firebase summon draft error:', err);
          alert('Failed to summon card online: ' + err.message);
        });
      }
    } else {
  ...
  function assignCardToRole(player, roleKey, char) {
    ...
    if (mpState.isOnline && mpState.roomRef) {
      ...
      mpState.roomRef.update(sanitizeForFirebase({
        ...
        console.error('assignCardToRole update error:', err);
      });
      return;
    }
  ...
  // Discard Card from Strict Reveal Flow
  function discardCard() {
    ...
    if (mpState.isOnline && mpState.roomRef) {
      ...
      const localId = getOrSetLocalPlayerId();
      if (activePlayer && activePlayer.id !== localId && !mpState.isHost) {
        alert(`⏳ It is currently ${activePlayer.name}'s turn to discard/assign.`);
        return;
      }
      ...
      mpState.roomRef.update(sanitizeForFirebase({
        ...
        console.error('discardCard update error:', err);
      });
      return;
    }
  ...
  function createOnlineRoom() {
    ...
    roomRef.set(sanitizeForFirebase(initialRoomData)).then(() => {
  ...
  function joinOnlineRoom(rawCode, rawName) {
    ...
    roomRef.once('value').then((snapshot) => {
      ...
      const players = ensureArray(room.players);
      ...
      if (existingIdx === -1) {
        ...
        roomRef.update(sanitizeForFirebase({
          players: players,
          budgets: budgets
        })).catch((err) => {
          console.error('joinOnlineRoom update error:', err);
        });
      }
  ...
  function handleRemoteRoomUpdate(snapshot) {
    ...
    if (data.status === 'lobby') {
      ...
      if (countEl) countEl.textContent = `${ensureArray(data.players).length} / ${data.maxPlayers || 4} Players`;
      ...
      if (grid) {
        grid.innerHTML = '';
        ensureArray(data.players).forEach((p, idx) => {
    ...
    if (data.status === 'drafting') {
      ...
      const myPlayer = ensureArray(data.players).find((p) => p.id === localId);
      ...
      document.getElementById('active-room-player-count').textContent = `${ensureArray(data.players).length} Players Online`;
      ...
      state.pool = ensureArray(data.pool);
      state.archive = ensureArray(data.archive);
  ...
  function placeAuctionBid(increment, isCustom = false) {
    ...
    mpState.roomRef.child('currentAuction').update(sanitizeForFirebase({
      currentBid: newBid,
  ...
  function resolveAuction(manualSold = false, manualPass = false) {
    ...
    if (auctionData.highestBidderId && !manualPass) {
      ...
      mpState.roomRef.child('currentAuction').update(sanitizeForFirebase({
        ...
        console.error('resolveAuction sold error:', err);
      });
      return;
    }
    ...
    mpState.roomRef.update(sanitizeForFirebase({
      ...
      console.error('resolveAuction discard error:', err);
    });
  }
  ...
  function assignAuctionWonCard(roleKey) {
    ...
    mpState.roomRef.update(sanitizeForFirebase({
      ...
      console.error('assignAuctionWonCard update error:', err);
    });
  }
  ...
  function launchOnlineMatch() {
    ...
    mpState.roomRef.update(sanitizeForFirebase({
      ...
      console.error('launchOnlineMatch update error:', err);
    });
  ...
  function leaveOnlineRoom(removeFromDb = true) {
    if (mpState.roomRef) {
      ...
      if (removeFromDb) {
        const localId = getOrSetLocalPlayerId();
        if (mpState.isHost) {
          mpState.roomRef.remove().catch(err => console.error('remove room error:', err));
          ...
          mpState.roomRef.child('players').set(sanitizeForFirebase(remainingPlayers)).catch(err => console.error('leave room error:', err));
        }
      }
  ...
  function setupEventListeners() {
    ...
    document.getElementById('mp-lobby-auction-toggle')?.addEventListener('change', (e) => {
      if (mpState.isOnline && mpState.isHost && mpState.roomRef) {
        ...
        mpState.roomRef.update(sanitizeForFirebase({ isAuctionMode: isAuction })).catch(err => console.error('lobby auction toggle error:', err));
        playSound('click');
      }
    });
    document.getElementById('mp-lobby-timer-select')?.addEventListener('change', (e) => {
      if (mpState.isOnline && mpState.isHost && mpState.roomRef) {
        const timer = parseInt(e.target.value, 10);
        mpState.roomRef.update(sanitizeForFirebase({ bidTimerDuration: isNaN(timer) ? 15 : timer })).catch(err => console.error('lobby timer error:', err));
        playSound('click');
      }
    });
    document.getElementById('mp-lobby-budget-select')?.addEventListener('change', (e) => {
      if (mpState.isOnline && mpState.isHost && mpState.roomRef) {
        const budget = parseInt(e.target.value, 10) || 200000;
        mpState.roomRef.update(sanitizeForFirebase({ startingBudget: budget })).catch(err => console.error('lobby budget error:', err));
        playSound('click');
      }
    });
    document.getElementById('mp-lobby-universe-select')?.addEventListener('change', (e) => {
      if (mpState.isOnline && mpState.isHost && mpState.roomRef) {
        ...
        mpState.roomRef.update(sanitizeForFirebase({
          universe: uni,
          pool: JSON.parse(JSON.stringify(charsList))
        })).catch(err => console.error('lobby universe error:', err));
        playSound('click');
      }
    });
